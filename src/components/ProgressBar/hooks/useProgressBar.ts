import {
  ...
  ProgressState,
  StroyTypes,
} from '@/components/StoryModal/types';
...
const useProgressBar = ({
  ...
  const inActiveColor = colors.textGray400;

  const barActiveColor = props?.barStyle?.barActiveColor ?? activeColor;
  const barInActiveColor = props?.barStyle?.barInActiveColor ?? inActiveColor;
